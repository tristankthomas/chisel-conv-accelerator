...
versions = ['v1\nNaive', 'v2\nPipelined\nRequests', 'v2.1\n64-bit\nRequests', 'v3\nOutput\nStreaming', 'v4\nLine\nBuffer', 'v5\nSpatial x4']
cycles = [7329, 3171, 1622, 1373, 1137, 607]
...
parallelism = ['×1', '×2', '×4']
total_cycles = [1137, 627, 607]
compute_cycles = [1024, 512, 256]
...
for i, (comp, mem) in enumerate(zip(compute_cycles, memory_overhead)):
    total = comp + mem
    ax4.text(i, total + 15, f'{total:,}', ha='center', va='bottom', fontsize=10, fontweight='bold')

for i, comp in enumerate(compute_cycles):
    ax4.text(i, comp / 2, f'{comp}', ha='center', va='center', fontsize=9, color='white', fontweight='bold')

for i, (comp, mem) in enumerate(zip(compute_cycles, memory_overhead)):
    if mem > 30:
        ax4.text(i, comp + mem / 2, f'{mem}', ha='center', va='center', fontsize=9, color='white', fontweight='bold')
...
pres_versions = ['v1\nNaive', 'v2\nPipelined\nRequests', 'v3\n64-bit\nRequests', 'v4\nOutput\nStreaming', 'v5\nLine\nBuffer', 'v6\nSpatial x4']
pres_cycles = [7329, 3171, 1622, 1373, 1137, 607]
...
for i, (comp, mem) in enumerate(zip(compute_cycles, memory_overhead)):
    total = comp + mem
    ax5.text(i, total + 15, f'{total:,}', ha='center', va='bottom', fontsize=10, fontweight='bold')

for i, comp in enumerate(compute_cycles):
    ax5.text(i, comp / 2, f'{comp}', ha='center', va='center', fontsize=9, color='white', fontweight='bold')

for i, (comp, mem) in enumerate(zip(compute_cycles, memory_overhead)):
    if mem > 30:
        ax5.text(i, comp + mem / 2, f'{mem}', ha='center', va='center', fontsize=9, color='white', fontweight='bold')
