// conv_fp_verify.c
// test program for RoCC floating-point convolution accelerator
...
int main() {
    ...
    // initialize input and hw_output arrays
    for (int i = 0; i < N; i++)
        for (int j = 0; j < N; j++) {
    ...
    // verify hardware output against software model with ULP tolerance
    int errors = 0;
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
