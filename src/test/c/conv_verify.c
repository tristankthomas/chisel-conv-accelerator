// conv_verify.c
// test program for RoCC convolution accelerator
...
// start convolution via custom instruction 1
...
// page-aligned memory buffers for hardware access
...
int main() {
    ...
    // initialise input and hw_output arrays
    for (int i = 0; i < N; i++)
        for (int j = 0; j < N; j++) {
            input[i][j] = prng_next();
            hw_output[i][j] = 0;
    ...
    unsigned long status;
    do { status = conv_poll(); } while (!(status & 0x1)); // wait for completion bit
    hw_end = rdcycle();
    ...
    // run software reference model and measure cycles
    sw_start = rdcycle();
    int pad = K / 2;
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            ...
            // shift accumulator to fit in 16-bit output
            sw_output[i][j] = (uint16_t)(acc >> 8);
        }
    }
    sw_end = rdcycle();

    // verify hardware output against software model
    int errors = 0;
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
    ...
    // print final test results and speedup
